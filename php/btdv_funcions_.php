...
//        error_log($compte . " AND " . $compteid);
    ...
    function GetBox($mysqli,$id,$ext_lang="")
    {
        ...
        {
            case 'es':
                $sql="SELECT name_es,type,description_es,price_es,quotes_es,activities,details_es,reservation_es,n_min,n_max,lat,lng,url,event_type,data,n_total,cellers,patrimoni,visites,rutes,promo_quant,promo_id,no_online,special_img,extra_fields,img,collaboradors,qr_img,persons_per_ticket,res_days,close_time,propietari,sessio_unica,ocult,aavv,mail_aux,com_obl,com_aux,linksessions,recordatori_es,xaccept,xaccept_description_es,taquilla_tancada,productes,enviament_id,pagament,enviament_str FROM box_data WHERE id=$id";
                break;
            
            case 'en':
                $sql="SELECT name_en,type,description_en,price_en,quotes_en,activities,details_en,reservation_en,n_min,n_max,lat,lng,url,event_type,data,n_total,cellers,patrimoni,visites,rutes,promo_quant,promo_id,no_online,special_img,extra_fields,img,collaboradors,qr_img,persons_per_ticket,res_days,close_time,propietari,sessio_unica,ocult,aavv,mail_aux,com_obl,com_aux,linksessions,recordatori_en,xaccept,xaccept_description_en,taquilla_tancada,productes,enviament_id,pagament,enviament_str FROM box_data WHERE id=$id";
                break;
            
            default:
                $sql="SELECT name,type,description,price,quotes,activities,details,reservation,n_min,n_max,lat,lng,url,event_type,data,n_total,cellers,patrimoni,visites,rutes,promo_quant,promo_id,no_online,special_img,extra_fields,img,collaboradors,qr_img,persons_per_ticket,res_days,close_time,propietari,sessio_unica,ocult,aavv,mail_aux,com_obl,com_aux,linksessions,recordatori,xaccept,xaccept_description,taquilla_tancada,productes,enviament_id,pagament,enviament_str FROM box_data WHERE id=$id";
                break;
        }
        $result=$mysqli->query($sql);
        $box = null;
    ...
    function GetReservationList($mysqli,$user_id=-1)
    {
        ...
        if($user_id!=-1)
        {
            $result = $mysqli->query("SELECT res.* FROM reserva AS res,box_data AS bdata WHERE res.box_id=bdata.id AND bdata.propietari=$user_id ORDER BY res.data DESC, res.id DESC");
        }
        else
        {
            $result = $mysqli->query("SELECT * FROM reserva ORDER BY data DESC, id DESC");
        }                
...
//            }
    ...
    function InsertBox($mysqli,$id,$name,$description,$details,$use,$price,$type,$quotes,$img,$activities,$n_min,$n_max,$lat,$lng,$etype,$edate,$etotal,$cellers,$patrimoni,$visites,$rutes,$destacat,$nou,$ocult,$no_online,$special_img,$qr_img,$extra_fields,$min_price,$name_es,$description_es,$details_es,$use_es,$quotes_es,$price_es,$name_en,$description_en,$details_en,$use_en,$quotes_en,$price_en,$ppt,$collaboradors,$sessions,$res_days="",$close_time=12,$propietari=0,$sessio_unica="",$aavv=false,$mail_aux="",$com_obl=false,$com_aux="",$linksessions=-1,$recordatori="",$recordatori_es="",$recordatori_en="",$xaccept=false,$xaccept_description="",$xaccept_description_es="",$xaccept_description_en="",$taquilla_tancada=false,$portada_btiquets=true,$productes="",$enviament_id=0,$pagament=1,$enviament_str="")
    {
        ...
        if($id==-1)
        {
            $sql="INSERT INTO box_data(name,description,details,img,price,type,url,quotes,activities,reservation,n_min,n_max,lat,lng,event_type,data,n_total,cellers,patrimoni,visites,rutes,destacat,nou,ocult,no_online,special_img,qr_img,extra_fields,min_price,name_es,description_es,details_es,reservation_es,quotes_es,price_es,name_en,description_en,details_en,reservation_en,quotes_en,price_en,collaboradors,persons_per_ticket,propietari,res_days,close_time,aavv,mail_aux,com_obl,com_aux,linksessions,recordatori,recordatori_es,recordatori_en,xaccept,xaccept_description,xaccept_description_es,xaccept_description_en,taquilla_tancada,portada_btiquets,productes,enviament_id,pagament,enviament_str) VALUES ('$name','$description','$details','$img','$price','$type','$url','$quotes','$activities','$use','$n_min','$n_max','$lat','$lng','$etype','$edate','$etotal','$cellers','$patrimoni','$visites','$rutes','$destacat','$nou','$ocult','$no_online','$special_img','$qr_img','$extra_fields','$min_price','$name_es','$description_es','$details_es','$use_es','$quotes_es','$price_es','$name_en','$description_en','$details_en','$use_en','$quotes_en','$price_en','$collaboradors','$ppt','$propietari','$res_days','$close_time','$aavv','$mail_aux','$com_obl','$com_aux','$linksessions','$recordatori','$recordatori_es','$recordatori_en','$xaccept','$xaccept_description','$xaccept_description_es','$xaccept_description_en','$taquilla_tancada','$portada_btiquets','$productes','$enviament_id','$pagament','$enviament_str')";
        }
        else
        {   
...
function AdminSession($mysqli,$sessionstr,$id)
{
    ...
    function EditCompte($mysqli,$id,$name,$user,$merchantcode,$terminal,$currency,$key,$url,$btype_0,$btype_1,$btype_2,$btype_3,$btype_4,$btype_5,$btype_6,$btype_7,$mail,$lopd,$bizum)
    {        
        $ret=-1;
    ...
    function InsertReservation($mysqli,$id,$box_id,$user_id,$comentaris,$quant_str,$num_reserva,$total,$data,$state,$quant_total,$tipus,$data_r,$data_e,$session_id=-1,$bnom="",$bdesc="",$rnom="",$rmail="",$rtel="",$rmun="",$regal=false,$text_regal="",$caducitat=6,$aavv=false,$data_e_out=null,$newsletter=false,$pagament=1,$addr1="",$addr2="",$cp="")
    {
        $ret=-1;
...
//                    $html = $html . $iva_1 . '% ' . translate("IVA INCLÒS", $lang);
    ...
    function GeneratePDF5($info_reserva,$num)
    {
        //include_once '../plugins/mpdf/mpdf.php';
        require_once '../vendor/autoload.php';
        ...
        <div class="pdf_xec">
            <div style="margin-left:-200px;width:1962px;">
                <div style="margin-top:70px;font-size:150px;">'.$num.'</div>
            </div>              
        </div>
        </body></html>';
...
//        return 1;
    ...
    function SendConfirmation6_sp2($mysqli,$info_reserva,$dmail="",$pdf=false)
    {
        ...
        if($pdf)
        {
            ...
            for($i=1; $i<=$qtotal;$i++)
            {
                $mpdf = GeneratePDF5($info_reserva,$comptador);
